...
def modular_bivariate(N, e, delta, s, desired_solution, roots_method="resultants"):
    ...
    More information: Zheng M., Kunihiro N., Hu H., "Cryptanalysis of RSA Variants with Modified Euler Quotient" Section 3
    :param N: the modulus
    ...
    pr = ZZ["x", "y", "z"]
    x, y, z = pr.gens()
    qr = pr.quotient(1 + x * y - z)
    X = int(2 * e * N ** delta / (N ** 2))
    Y = int(3 * N)
    Z = X * Y + 1
    ...
    i = 0
    for k in range(s + 1):
        for j in range(1, floor(tau * k) + 1):
            logging.debug(f"indices: {i = }, {j = }, {k = }")
            h = x ** i * y ** j * f ** k * e ** (s - k)  
            h = qr(h).lift()
            shifts.append(h)
    ...
    fr = f.parent()
    x, y = fr.gens()
    polynomials = reconstruct_polynomials(L, f, e ** s, monomials, [X, Y, Z], preprocess_polynomial = lambda p: p(x, y, 1 + x * y))
...
def generate_MEQRSA_instance(modulus_bit_length, delta, max_attempts = 10):
    """
    Generate an MEQ RSA instance with given bit-lengths of the modulus and private key d
    ...
    :param max_attempts: the maximum number of attempts to generate MEQ RSA instance. (default: 10)
    ...
    while attempts < max_attempts and N.nbits() != modulus_bit_length and d.nbits() != d_bit_length:
        ...
        logging.debug(f'p: {p}')
        logging.debug(f'q: {q}')
        return MEQRSA_instance, desired_solution
    logging.warning(f"Failed to generate MEQ RSA instance after {max_attempts} attempts...")
    return None


def attack_MEQRSA_instance(modulus_bit_length, delta, s = 3):
    """
    Small private key attack on MEQ RSA instance with given parameters
    :param modulus_bit_length: the bit length of the modulus.
    ...
    """
    result = generate_MEQRSA_instance(modulus_bit_length, delta)
    if result is not None:
        MEQRSA_instance, desired_solution = result
    else:
        print(f"Sorry, cannot generate an MEQ RSA instance...")
        return 0, 0
    N, e = MEQRSA_instance[0], MEQRSA_instance[1]
    ...
    if solution is not None:
        ...
        print(f"Sorry, cannot attack this MEQ RSA instance...")
        return 0, test_time
...
if __name__ == "__main__":
    ...
    result, test_time = attack_MEQRSA_instance(modulus_bit_length, delta, s)
    if result:
